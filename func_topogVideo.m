function [EEG_topo_video] = func_topogVideo(EEG_psd_second,savePath)
    ...
    % load('psd_data_seconds_segmented_newProcessed_4')
%     segmentedDataFile = 'psd_data_seconds_segmented_newProcessed_intp_4.mat';
    % load(segmentedDataFile);
    plotFields = {'Aldi','Ikea','Optus','VicGov','Westpac','Woolworths','Wotif'};
    plotFields = cat(2,{'Pod_P1_Life_Uncut_Grab_1','Pod_P2_Life_Uncut_Grab_2','Radio_P1_2_Neuro_Part1_Part2','Radio_P3_Neuro_Part3','Social_Media'},{'MidRoll_Uber_Podcast';'MidRoll_VillageRoadshow_Podcast';'MidRoll_Westpac_Podcast';'MidRoll_Woolworths_Podcast';'PostRoll_A2_Podcast';'PostRoll_Amazon_Podcast';'PostRoll_KFC_Podcast';'PostRoll_Toyota_Podcast';'PreRoll_Disney_Podcast';'PreRoll_Lancome_Podcast';'PreRoll_Mcdonalds_Podcast';'PreRoll_Optus_Podcast';'Radio_Ads1_Amazon_Radio';'Radio_Ads1_Disney_Radio';'Radio_Ads1_KFC_Radio';'Radio_Ads1_Lancome_Radio';'Radio_Ads1_Mcdonalds_Radio';'Radio_Ads1_Optus_Radio';'Radio_Ads1_Westpac_Radio';'Radio_Ads1_Woolworths_Radio';'Radio_Ads2_A2_Radio';'Radio_Ads2_Aldi_Radio';'Radio_Ads2_Carlton_Radio';'Radio_Ads2_Ikea_Radio';'Radio_Ads2_Toyota_Radio';'Radio_Ads2_Uber_Radio';'Radio_Ads2_VicGov_Radio';'Radio_Ads2_VillageRoadshow_Radio';'Radio_Ads2_Wotif_Radio'}.');
    
    data_field_names = fieldnames(EEG_psd_second);    % get header names to find comp accuracy indx

    colIdxVector = [];
    for colnamei = 1:length(plotFields)

        colName = plotFields{colnamei};
        colIdx = find(strcmp(data_field_names, colName));   
        colIdxVector = [colIdxVector colIdx]; % aggregate column header indices (col)
    ...
    for pi = 1:length(EEG_psd_second)
        currentGroup = 'study'; %EEG_psd_second(pi).A_group;
        currentChannels = EEG_psd_second(pi).A_chanlocs;

        if length(currentChannels)<19
...
%         groupsIndx(currentGroup).group = currentGroup;
    ...
    for pi = allGroupIndices
        ...
        EEG_topo_video(pi).A_subject  = EEG_psd_second(pi).A_subject;
        EEG_topo_video(pi).A_group    = 'study'; % EEG_psd_second(pi).A_group;
        EEG_topo_video(pi).A_srate    = EEG_psd_second(pi).A_srate;
        EEG_topo_video(pi).A_chanlocs = EEG_psd_second(pi).A_chanlocs;
    ...
    %             end
                ...
                EEG_topo_video(pi).(fnames{fi}) = cat(3,theta_data,alpha_data,beta_data,attention_data);

        end     

        EEG_topo_video(pi).A_dataNames = {'theta_data','alpha_data','beta_data','attention_data'};
...
%         for averagei = 1:3 / start of averaging loop, enable only if
%         there are groups.

            groupinterval   = allGroupIndices; % groupsIndx(averagei).index;
            groupNumber     = 1; %groupsIndx(averagei).group;
...
%             for groupi = 1:3
                ...
                if isempty(EEG_topo_video_avg.(fieldindx{:}))
                    continue
                end
                % get data
                data = EEG_topo_video_avg.(fieldindx{:})(:,:,dataindx);
...
%                 currentGroup =  []; %['Group',num2str(EEG_topo_video_avg.A_group)];
    ...
    %                 subplot(3,3,subplotIndx)
                    ...
                    topoplot(data(secondi,:),EEG_topo_video_avg.A_channels,'style','map','plotrad',0.55,'maplimits',[minValue maxValue]); % channel info retrieved from external file. 
