function [EEG_topo_avg] = func_topoPlotFreq(EEG_psd_second,savePath)
    ...
    for pi = 1:length(EEG_psd_second)
        ...
        EEG_topo_freq(pi).A_group    = 'study'; %EEG_psd_second(pi).A_group;
...
%                 disp(size(EEG_topo_freq(pi).(fnames{fi})))
    ...
    cd(savePath)
    % load('topo_freq_data');
    % load('topo_freq_data_newProcessed');
    % load(topo_freq_data_fileName);

    groups = {EEG_topo_freq.A_group};

    groupVector = {};
    for gri = 1:3    
        groupIndices = {find([groups{:}]==gri)};   
        groupVector = cat(2,groupVector,groupIndices);
...
%             EEG_topo_avg(averagei).A_channels           = EEG_topo_freq(groupinterval(1)).A_chanlocs;
