function [EEG_topo_avg] = func_topoPlotMv(EEG_epoch,savePath)
...
for pi = 1:length(EEG_epoch)
    ...
    EEG_topo_mv(pi).A_group    = 'study'; %EEG_epoch(pi).A_group;
...
%     for averagei = 1:3
        ...
        thresholdPos = dataMedian + 2* dataSTD;
        thresholdNeg = dataMedian - 2* dataSTD;
        ...
        EEG_topo_avg.A_group = 1;
        EEG_topo_avg.A_channels = EEG_topo_mv(groupinterval(1)).A_chanlocs;
...
for fi = nonDataIndx:length(fnames)
  ...
  fprintf('\nPROCESSED PLOT: %s\n%s\n%s \n',fieldindx{:});  
    allgroups = cat(3,EEG_topo_avg.(fieldindx{:}));% histData; 
...
%     minValue = min( allgroupsCurrentData ,[],'all');
    ...
    registeryDirectory = [savePath '\plots\microvolts\grouped'];
